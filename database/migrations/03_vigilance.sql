...
CREATE TABLE signals (
    ...
    -- URL of the original source or evidence
    ips NUMERIC(5, 2),
    -- Impact Probability Score (for be defined)
...
CREATE TABLE trends (
    ...
    -- Date of the first signal associated with the trend
...
CREATE TABLE alerts (
    ...
    id_alert_origin SMALLINT REFERENCES alert_origins(id_alert_origin),
    -- Origin of the alert (e.g., internal, external, partner)
    publication_date DATE,
    activation_rule TEXT,
    -- Description of the rule or criteria for activating the alert (Regla de transición aplicada)
    notes TEXT,
    -- Additional notes or comments about the alert
    id_creator BIGINT REFERENCES users(id_user),
    -- User who created the alert
    id_validator BIGINT REFERENCES users(id_user),
    -- User who validated the alert
    validation_date DATE,
    -- Date when the alert was validated
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);
