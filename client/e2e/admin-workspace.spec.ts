...
test("una analista crea una señal y consulta el IPS calculado por el backend", async ({ page }) => {
  ...
  await page.getByLabel("Título").fill("Nueva capacidad regional");
  await page.getByLabel("Resumen").fill("Se amplía la capacidad de prueba.");
  await page.getByLabel("Fecha de publicación").fill("2026-07-13");
  await page.getByLabel("Tipo de señal").selectOption("TECH");
  await page.getByLabel("URL de evidencia").fill("https://example.test/nueva");
  await page.getByLabel("Categoría y FCV").selectOption("3");
  await page.getByLabel("Fuente").selectOption("8");
  await page.getByLabel("Impacto").selectOption("HIGH");
  await page.getByLabel("Urgencia").selectOption("MEDIUM");
  await page.getByLabel("Confiabilidad").selectOption("HIGH");
  await page.getByLabel("Alcance").selectOption("REGIONAL");
  await page.getByLabel("Palabras clave").fill("encapsulado, talento");
