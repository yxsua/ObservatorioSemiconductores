...
function SignalFilterForm({ catalogs, filters, onApply, onClear }: Props) {
  const [fcv, setFcv] = useState(filters.fcv ?? "");
  const [categoryId, setCategoryId] = useState(String(filters.categoryId ?? ""));
  const categories = catalogs?.categories.filter((option) => !fcv || option.fcvCode === fcv) ?? [];
  ...
  return <section aria-labelledby="signal-filters-title" className={styles.filters}>
    ...
    <form className={styles.filtersForm} key={JSON.stringify(filters)} onSubmit={submit}>
      ...
      <div><label htmlFor="signal-fcv">Factor crítico de vigilancia</label><select value={fcv} onChange={(event) => { setFcv(event.target.value); setCategoryId(""); }} id="signal-fcv" name="fcv"><option value="">Todos</option>{catalogs?.fcv.map((option) => option.code && <option key={option.code} value={option.code}>{option.name}</option>)}</select></div>
      ...
      <details className={styles.advancedFilters}><summary>Filtros avanzados</summary><div className={styles.advancedGrid}>
        <div><label htmlFor="signal-category">Categoría</label><select value={categories.some((option) => String(option.id) === categoryId) ? categoryId : ""} onChange={(event) => setCategoryId(event.target.value)} id="signal-category" name="categoryId"><option value="">Todas</option>{categories.map((option) => option.id && <option key={option.id} value={option.id}>{option.name}</option>)}</select></div>
