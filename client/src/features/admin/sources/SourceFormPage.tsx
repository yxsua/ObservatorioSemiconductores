...
function SourceForm({ initial, mode, source, types, token, onSaved }: { initial: Values; mode: "create" | "edit"; source?: Source; types: Array<{code:string;name:string}>; token: string; onSaved: (id:number)=>Promise<void> }) {
  ...
  return <section className={styles.page}><header className={styles.header}><div><h1>{mode === "create" ? "Nueva fuente" : `Editar ${source?.name ?? "fuente"}`}</h1><p>Registra el origen que podrá utilizarse como evidencia en las señales.</p></div></header><form className={styles.form} onSubmit={submit} noValidate>
    ...
    <section className={styles.section}><h2>Identificación</h2><div className={styles.grid}>
      <div className={styles.field}><label htmlFor="source-type">Tipo</label><select id="source-type" {...register("typeCode")}><option value="">Selecciona una opción</option>{types.map((type)=><option key={type.code} value={type.code}>{type.name}</option>)}</select>{errors.typeCode && <small className={styles.error}>{errors.typeCode.message}</small>}</div>
      <div className={styles.field}><label htmlFor="source-name">Nombre</label><input id="source-name" maxLength={200} {...register("name")} />{errors.name && <small className={styles.error}>{errors.name.message}</small>}</div>
      <div className={styles.field}><label htmlFor="source-country">País o región</label><input id="source-country" maxLength={100} {...register("country")} /></div>
      <div className={styles.field}><label htmlFor="source-reliability">Confiabilidad histórica (0–1)</label><input id="source-reliability" min="0" max="1" step="0.01" type="number" {...register("historicalReliability")} />{errors.historicalReliability && <small className={styles.error}>{errors.historicalReliability.message}</small>}</div>
    </div></section><section className={styles.section}><h2>Canales</h2><div className={styles.grid}>{([["website","Sitio web"],["rssUrl","URL RSS"],["apiUrl","URL API"]] as const).map(([name,label])=><div className={`${styles.field} ${styles.wide}`} key={name}><label htmlFor={`source-${name}`}>{label}</label><input id={`source-${name}`} maxLength={2000} type="url" {...register(name)} /></div>)}</div></section>
