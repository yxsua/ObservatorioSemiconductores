import { z } from "zod";
import {alertAssessmentSchema} from "@/features/assessment/assessment";
...
export const alertFormSchema=z.object({
  ...
  responseDeadline:z.union([z.literal(""),z.iso.date()]),assessment:alertAssessmentSchema.nullable().refine((v):boolean=>v!==null,"Completa la valoración estratégica."),
...
export const toAlertInput=(v:AlertFormValues)=>({title:v.title.trim(),executiveSummary:v.executiveSummary.trim(),implications:nullable(v.implications),recommendations:nullable(v.recommendations),responseDeadline:v.responseDeadline||null,assessment:alertAssessmentSchema.parse(v.assessment),activationRule:nullable(v.activationRule),notes:nullable(v.notes),signalIds:v.signalIds.map(Number),trendIds:v.trendIds.map(Number),audienceCodes:v.audienceCodes});
export function alertReadiness(v:AlertFormValues){return [
  {label:"Nivel de alerta asignado",ready:alertAssessmentSchema.safeParse(v.assessment).success},
